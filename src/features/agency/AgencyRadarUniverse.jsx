    ...
    <div className="relative mb-5 overflow-hidden rounded-xl bg-[#131A15] p-4">
      ...
      {worlds.length > 0 && (
        <div className="mb-2 flex justify-end">
          <select
            ...
            className={`appearance-none rounded-full border bg-[#131A15] px-3 py-1 font-mono text-[10px] outline-none ${
      ...
      {artists.length === 0 ? (
        <div className="py-10 text-center">
          <h3 className="font-display text-lg text-[#EEF4E7]">{S.rosterEmptyTitle}</h3>
          <p className="mx-auto mt-1 max-w-[300px] text-xs leading-relaxed text-[#AEB9AD]">{S.rosterEmptyBody}</p>
        </div>
      ) : (
        <div className="relative mx-auto aspect-square max-w-[400px]">
          ...
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
            <span className="mx-auto grid h-16 w-16 place-items-center rounded-full border-2 border-white/25 bg-[#1B241D] font-mono text-[9px] uppercase tracking-[0.08em] text-[#AEB9AD]">
              {shown.length}
            </span>
            <span className="mt-1.5 block font-mono text-[9px] uppercase tracking-[0.1em] text-[#89958A]">{S.rosterCenter}</span>
          </div>
          ...
          {shown.map((a, i) => {
            ...
            return (
              <button key={a.id}
                ...
                <span className={`relative mx-auto grid h-14 w-14 place-items-center overflow-visible rounded-full border-2 bg-[#1B241D] transition-transform hover:scale-105 ${RING[st.key]} ${st.pending > 0 ? 'shadow-[0_0_18px_rgba(200,240,77,0.25)]' : ''}`}>
                  {a.photo_url
                    ? <img src={a.photo_url} alt="" className="h-full w-full rounded-full object-cover" />
                    : <span className="font-display text-base text-[#EEF4E7]">{(a.stage_name || '?').slice(0, 1)}</span>}
                  {st.pending > 0 && (
                    <span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-accent px-1 font-mono text-[9px] font-bold text-ink">✦{st.pending}</span>
                  )}
                </span>
                <span className="mt-1.5 block w-20 truncate font-mono text-[8px] uppercase tracking-[0.06em] text-[#AEB9AD]">
                  {a.stage_name}
                </span>
              </button>
            )
      ...
      {selected && (
        <div className="fixed inset-0 z-40 flex flex-col bg-paper animate-fade-in" role="dialog" aria-modal="true" aria-label={selected.stage_name}>
