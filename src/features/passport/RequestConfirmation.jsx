...
export default function RequestConfirmation() {
  ...
  return (
    <PageShell max="max-w-sm">
      <Wordmark className="justify-center mb-8" />
      <div className="card text-center animate-fade-in">
        <p className="text-3xl mb-3">✅</p>
        <h1 className="text-xl font-bold text-soft mb-2">{T.request.confirmTitle}</h1>
        <p className="text-muted mb-4">{T.request.confirmBody(artistName)}</p>
        ...
        {waUrl && <p className="text-xs text-muted">{T.request.whatsappHint}</p>}
      </div>
    </PageShell>
  )
}
