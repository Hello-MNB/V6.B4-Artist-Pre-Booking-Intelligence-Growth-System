...
- **Deploy reliability (open):** production auto-deploy from main is UNRELIABLE for BOTH Vercel projects (GitHub 9 Jul: recent deploys landed as Preview; Production–gigproof-website 2h stale). Durable fix (Cowork/owner dashboard): each project → Settings→Git→Production Branch=main + auto-deploy on; then pushes auto-promote. Interim: site deploy hook (POST) + app manual Redeploy. Sandbox can't reach live sites (403) — can't visually verify from container.

## Migrations
- **029 WRITTEN, NEEDS APPLYING (Cowork):** artists.whatsapp_share + SECURITY DEFINER get_shared_whatsapp(). Artist-controlled WhatsApp sharing (owner ruling: artist decides in Settings; default OFF). App degrades gracefully pre-apply (no CTA). RESOLVES the old dead WhatsApp-CTA finding.
