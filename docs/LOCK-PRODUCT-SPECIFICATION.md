...
### 8.10 Representation — Roster cockpit (real routes `/agency` + `/agency/requests` · `/agency/radar` · `/org/members`)

_Deepened to the full interactive standard, 21 Jul — TEAM IX-ENTITIES. Grounded in the BUILT code: `AgencyDashboard.jsx` (477 lines), `AgencyRadarUniverse.jsx` (175 lines, embedded), `rosterNextAction.js` (81 lines), `AgencyRequestsInbox.jsx` (173 lines), `RadarFeed.jsx` (142 lines), `org/Members.jsx` (189 lines, shared). **Route-name correction (flagged):** the prior spec header cited `/representation/roster` + `/reqs`/`/radar`/`/team` — no such routes exist. The real, running routes are `/agency`, `/agency/requests`, `/agency/radar`, and `/org/members` (`App.jsx` lines 203–205, 171). "Representation" is the canon UI/entity name (§3.4); `agency` is the frozen code identifier underneath (§0.2 rule 5) — both are correct, at different layers; this heading now shows the real route so a builder can find the file.

**THE ONE JOB.** The manager opens ONE screen and knows, for the whole roster, which artist needs them right now and what the one bound action is — never a table to scan. **1-screen happy path (§6 law 13):** land on Roster (`/agency`) → the embedded roster-universe + the consented-grant list + the owned-artist list each surface ONE next-action chip per artist, computed from that artist's real state → tap the chip → it deep-links straight to the thing to do (the Passport, or the Requests inbox pre-filtered to that artist) → act → return.

**PURPOSE.** A roster **cockpit** (not a CRM table): one next-best action per artist, computed from real state, bound to that artist's route. Access to a consented artist is an ArtistAccess **grant** the artist can revoke — never ownership; an **owned** roster row (added directly, no artist account) has no such grant to revoke.

**NAV IN / OUT (per §7.7.a + `navItems.jsx` + `App.jsx`, verified — one real mismatch flagged).** **In:** login → `RoleHome` → `ROUTES.agency` (`/agency`) for role `AGENCY` (unless the active org's `workspace_type==='producer'`, which routes to `/production` instead, §8.11). **Bottom/side nav (BUILT, `navItems.jsx` agency branch) is exactly 4 tabs:** Roster (`/agency`, default) · Radar (`/agency/radar`) · Requests (`/agency/requests`) · Account (`/settings`). **Flagged mismatch:** §7.3's prose ("Representation: Roster · Requests · Radar · **Team**") and the entity map's shorthand both name a 4th nav tab, "Team" — **no such tab exists in the built nav.** Team/seats live at `/org/members` (`Members.jsx`, shared across every workspace type, §17.B.6), reached only via (a) the top-right hub's account actions (§7.2, universal) or (b) an in-page link from Roster's first-run checklist (`to="/org/members"`, visible only until dismissed). There is no persistent nav-in to Team from the Roster screen once the checklist is dismissed — a genuine nav-completeness gap, not invented (verified by reading `navItems.jsx` line 27-32 against §7.3's own text). **Out:** each screen carries its own back link to `/agency` (`AgencyRequestsInbox`, `RadarFeed`) or to `/` (`Members`); no dead ends.

**DESKTOP & MOBILE layout / no-scroll target.** `PageShell max-w-4xl`, light theme throughout (no dark island on this entity — Representation is not Radar/Passport). Desktop: `lg:grid-cols-[minmax(0,1fr)_300px]` — roster list (main) + a 300px `RequestsSideCard` (incoming-requests preview, side). Mobile: the side card collapses to full-width, stacking below the roster. **No-scroll target:** this screen is a genuinely long list page (roster-universe widget + access-requests + consented grants + checklist + owned roster + add form), so the no-scroll law (§6 law 7) does **not** apply as "one viewport" here — it is a single continuously-scrolling page, which is the correct shape for a variable-length roster (contrast with Radar/Passport, which ARE bounded to one viewport). The one **named contained-scroll region** on this screen is the embedded roster-universe orbit (`AgencyRadarUniverse`, `aspect-square max-w-[400px]`) — a fixed-height widget, never itself scrolling, that truncates to the first 10 artists with an honest "+{overflow} more — full roster below" jump-link rather than silently hiding them.

**COMPONENTS (BUILT, `AgencyDashboard.jsx`, top → bottom).**
1. Page head: H1 "Roster" (`T.agency.title`) + two quiet links ("Radar ›", "Requests ›" to the two secondary routes).
2. **`AgencyRadarUniverse`** — an embedded roster-as-universe widget (distinct from both the artist Radar §8.2 and the `/agency/radar` RadarFeed below — three different "radar" surfaces in this product, named consistently in code but worth a builder's attention): up to 10 artists rendered as orbiting "worlds" around a center "roster" node; ring color = bounded state (`needs`/`established`/`developing`/`missing`, derived from pending-unapproved-claims vs verified/supporting-approved vs raw signal count); a ✦ badge = the artist's own pending-claim COUNT (an inbox count, never a grade); a "worlds" dropdown filters by genre/content-world as a **coverage lens** (dims non-matching to 25%, never removes); tap an artist → a full-screen overlay (evidence-picture `StatusChip`, "{n} to review" if pending, "View Passport" or "Not published yet").
3. **`AccessRequestsCard`** — the ArtistAccess grants THIS org has requested against artists it does not own (pending / active / revoked), each with real scope chips + territory + expiry from the actual grant row (migration 027) — RLS hides the artist entirely until they approve, so a pending row has nothing to leak.
4. **Consented-roster card** — renders only once ≥1 active grant exists (migration 032): each row = artist name + city + scope chips + **one** `NextActionChip` (below).
5. **First-run checklist** (dismissible, `localStorage`-persisted) — "add an artist" / "invite your team" (→ `/org/members`) / "publish a Passport", each a plain checkable line, never shaming.
6. **Owned-roster list** — artists added directly (no grant to revoke): avatar/photo · stage name · genre + published/draft tag · "updated {date}" · a `StatusChip` (`rosterStatus()`) + one `NextActionChip`.
7. **Add-artist form** — a 2-tab toggle: **Invite** (canon-correct: paste a Passport link/id → `requestArtistAccess()` creates a `pending` grant; territory + scope checkboxes `upload/edit/share/publish` — `view` is the always-included floor) vs **Own** (legacy placeholder: `stage_name` + `genre` only, no artist account, no consent to request).
8. **`RequestsSideCard`** — the 3 most recent incoming availability requests (any status), a "{n} new" badge on unread, "Open inbox ›" to `/agency/requests`.

**BUILT vs the previous spec's roster-card narrative (flagged, not invented).** The prior spec described each roster row as a rich narrative card — "what changed" line + a `why` sentence + urgent-buyer framing directly on the card. **That narrative IS built — but on the separate `/agency/radar` (RadarFeed) screen, not on the Roster rows themselves.** RadarFeed's cards (below) carry exactly this shape: an icon + `StatusChip` + an `explain()`-generated one-sentence match + a method label + an evidence-basis line + one action button. The Roster screen's own rows are flatter: a `StatusChip` (strong/developing/missing) + a text-only `NextActionChip` — no found/what-changed prose, no explicit urgent-buyer sentence on the row itself (urgency is conveyed only via the chip's accent-colored styling when the action is `nextReplyRequest`). Builders should treat RadarFeed as "the roster's narrative layer" and Roster as "the roster's action layer" — today they are two screens, not one card doing both jobs.

**Two independent "roster health" derivations coexist on ONE page (flagged consistency gap).** `AgencyRadarUniverse`'s ring color uses `artistState()` (needs/established/developing/missing, based on pending vs approved claims). The owned-roster row below it uses `rosterStatus()` (strong/developing/missing, based on a count of `lineup_frequency_band`/`sells_tickets`/`price_band`/`photo_url` being set). Both are firewall-safe (categorical, never a number-as-grade) but they are **different rules** rendering simultaneously for the same artist on the same screen — they can disagree (e.g. an artist with 3 profile fields set but zero claims reviewed reads "established" in the universe ring and "developing" in the roster row). Not a firewall violation; a maintainability/consistency gap worth a T-number to unify.

**ENUMERATED INTERACTIVITY — the next-best-action ladder (`rosterNextAction.js:pickRosterAction`, the one mechanism behind every `NextActionChip`):**
| # | Real-state condition | Action shown | Route | Urgent styling | Scope-gated? |
|---|---|---|---|---|---|
| 1 | ≥1 open (`status='new'`) availability request for this artist | "Reply to request" | `/agency/requests?artist={id}` (auto-opens that artist's newest new request) | **yes** (accent border) | no — always allowed |
| 2 | not published | "Publish Passport" (or "View Passport" if the grant lacks `publish` scope) | `/passport/{id}` | no | yes — needs `publish` scope on consented rows |
| 3 | newest evidence item >90 days old (`FRESHNESS_DAYS`) | "Refresh proof" | `/passport/{id}` | no | yes — needs `upload` scope |
| 4 | no link/video evidence at all | "Request video" | `/passport/{id}` | no | no (view floor) |
| 5 | published + fresh | "Share Passport" | `/passport/{id}` | no | yes — needs `share` scope |
| 6 | else | "View Passport" | `/passport/{id}` | no | no (always allowed) |
Owned rows pass `scope: null` (full control, nothing gated); consented rows are gated to exactly what the grant's `scope[]` allows — the ladder never shows an action the grant doesn't permit.

**ENUMERATED INTERACTIVITY — every other interactive element:**
| Element | Trigger | In-place behavior |
|---|---|---|
| Roster-universe "worlds" filter | select | dims non-matching artist nodes to 25%, in place, reversible |
| Roster-universe artist node | tap | full-screen overlay (dialog role), "← Back to universe" exits in place |
| Overflow "+{n} more" link | tap | anchor-jumps down to the owned-roster list (`#roster`), same page |
| Access-request "Cancel invite" / "Revoke" | tap → `window.confirm` | `revokeArtistAccess()`, row updates in place |
| Add-artist mode toggle (Invite/Own) | tap | swaps the form fields in place, no navigation |
| Scope checkboxes (upload/edit/share/publish) | tap | `publish` checked → an inline hint appears ("this lets them publish on your behalf") |
| "Send invitation" / "Save" | tap | `requestArtistAccess()` / `upsertArtist()`; success = toast + the new/updated row briefly highlights (`ring-accent`, 4s) — never a silent reload |
| First-run checklist dismiss | tap | persists to `localStorage`, checklist disappears for this browser |
| `AgencyRequestsInbox` row | tap | accordion-expands ONE row at a time in place (`openId` state) — never a second page |
| "Mark replied" / "Mark closed" | tap ("closed" requires a second inline confirm chip, never a modal) | `updateRequestStatus()`, row status chip updates in place |
| `?artist=` deep-link (from a roster chip) | on load | filters the inbox to that artist + auto-opens their newest `new` request once, never re-fires |
| RadarFeed artist/type filters | select | client-side filter, triage only — never a ranking sort (code comment: "Filters are triage only") |
| RadarFeed signal card action button | tap | routes to `/agency/requests` (respond signals) or `/passport/{artistId}` (all others) — never a dead `/evidence/:id` link (that route is artist-only; a prior bug routed agency signals there and dead-ended, now fixed) |

**STATES.** Roster: loading · error (`ErrorState` + retry) · empty (no artists — CTA "Add artist" + "Invite your team") · populated. Consented-grant row: pending · active · revoked (via `AccessRequestsCard`). Owned row: `StatusChip` strong/developing/missing. Requests inbox row: new · replied · closed (with an inline "closing hides the request, so ask once" confirm). RadarFeed: loading · error · genuinely-empty ("the quiet is real, not a swallowed failure" — code comment) · filtered-to-empty (with a "clear filters" ghost).

**SIGNALS FIRED — none, on any of these three screens.** `grep` of `AgencyDashboard.jsx`, `AgencyRequestsInbox.jsx`, and `RadarFeed.jsx` for `logEvent(` returns **zero matches**. This confirms §7.7.a's flag verbatim: "Relationship (grants — ⚠ **NOT-WIRED** §21.1)". Every mutation on these screens (invite sent, grant revoked, request marked replied/closed, artist added) changes real DB state but fires **no** analytics event — the operator console (§8.12) cannot see Representation activity in any funnel or Gate tile. This is the single largest unwired signal gap in the product's canon event list.

**LEXICON (EN + HE, `agency.*` + `org.*` + `access.*` + `radar.universe.*` namespaces — all ✓ shipped).** Representative keys: `agency.title` / `consentedTitle` / `consentedHint` / `addArtist` / `empty` / `requests` / `inviteTabInvite` / `inviteTabOwn` / `inviteFieldLabel` / `inviteScopeLabel` / `migration027Note` / `nextReplyRequest` / `nextPublishPassport` / `nextViewPassport` / `nextRefreshProof` / `nextSharePassport` / `nextRequestVideo` / `filteredForArtist(name)` / `showAllRequests` / `statusNew` / `statusReplied` / `statusClosed` / `openInbox` / `addedToRoster(name)`. HE confirmed present verbatim (e.g. `nextReplyRequest` → "השב לבקשה", `addArtist` → "הוסף אמן", `consentedTitle` → "רוסטר בהסכמה"). No §15.4 localization-matrix entry exists yet for `agency.*`/`production.*`/`admin.*` (§15.4 currently documents only onboarding/radar/passport/request/producer/booker/settings/cookie namespaces) — **flagged gap**: the localization matrix (§15.4, out of this team's territory — §8.9–8.13 only) should eventually gain a Representation/Production/Admin row set to match what is actually shipped here.

**FIREWALL / entity law.** No roster **rank** — both status derivations are categorical, never a number-as-grade. Reaction insight = method-safe text only (RadarFeed's `explain()` lines are rule-derived sentences, never a score). No CRM tables. ArtistAccess = grant, revocable by either side, never ownership language (confirmed in code comments and copy: "a grant... never ownership").

**DEFINITION OF DONE.** Roster surfaces one next-action chip per artist from real, scope-gated state; consented vs owned rows both render; access-request lifecycle (pending/active/revoked) visible + revocable; requests inbox supports artist-deep-link auto-open; RadarFeed's narrative cards are the roster's "what changed + why" layer; Team reachable (today: hub + checklist only, not a nav tab — flagged); no rank, no CRM table, no ownership language; zero analytics signals fired (flagged, not fixed here).

**PART-8 SHAPE.**
- **SUCCESS FORMULA.** This cockpit succeeds when a manager with 15 artists opens it and, within seconds, knows exactly which ONE artist needs them and what to click — never when they have to read a table and infer priority themselves. It fails if two artists in equal need render with visibly different "health" simply because two different rules computed their badge.
- **COMPLETENESS CHECKLIST.** Nav-in/out incl. the Team-tab mismatch ✓ · next-best-action ladder enumerated with scope-gating ✓ · every interactive element listed ✓ · states per surface ✓ · signals fired (zero, flagged) ✓ · EN+HE lexicon ✓ · firewall (no rank, grant-not-ownership) ✓ · no-scroll target defined for a genuinely long page ✓ · the two-derivation consistency gap named ✓.
- **PARAMETER AUDIT (vs HEAD, this pass).** Taxonomy/terminology: **present** (Representation vs `agency` code identifier already reconciled at §0.2/§3.4). Entities: **present** (ArtistAccess grant model, §3.3). Interactivity enumeration: **was THIN** (roster-card narrative described but not actually where it's built; the 4th "wrong-tab" Team claim uncaught) → **backfilled in place**, `ratify: R00`. Nav map: **was THIN/wrong** (route names didn't match code; Team-tab claim didn't match `navItems.jsx`) → **corrected in place** this pass. Signals: **was silently absent** (§7.7.a already flagged NOT-WIRED, but no §8.10 prose said so) → **stated explicitly** here. Design-system tokens: unchanged.

---

### 8.11 Production — Lineup board (real routes `/production` · `/production/events` · `/production/requests`)

_Deepened to the full interactive standard, 21 Jul — TEAM IX-ENTITIES. Grounded in the BUILT code: `ProductionDashboard.jsx` (238 lines, one component rendering all three routes) + `navItems.jsx` (producer branch) + `orgs.js` (`getMembers`, `listOrgGigs`, `groupGigsIntoEvents`, `listProductionRequests`). **§8.11's C6-gated creation flows stay owner-gated per this team's brief — nothing below authorizes building them.**

**THE ONE JOB.** A production company opens one workspace and reads, at a glance, its team, its booked lineup slots by event, and the availability requests it has sent — today a **read surface**, not yet a create/act surface (flagged honestly below; do not infer more interactivity than is built). **1-screen happy path (BUILT, today):** land on Team (the actual default) → tab to Events → scan slot chips per event → tab to Requests → scan reply status. There is currently no "next action" to take from inside this workspace at all — every mutation (confirming a slot, replying to a request, booking an act) happens **outside** it, on a Passport or in `/org/members`.

**PURPOSE.** An event/lineup **board** (not rows): each event holds lineup slots; the intended flow is to fill a slot by requesting availability or confirming a known act, and to *book* by acting as a Buyer (opening a Passport → availability request). **BUILT reality (verified in code): none of the slot-filling, slot-confirming, or booking interactions exist yet** — see the honesty section below.

**NAV IN / OUT (per §7.7.a + `navItems.jsx` + `App.jsx`, verified — one default-tab correction flagged).** **In:** login → `RoleHome` → `role===AGENCY && isProducerWorkspace` → `ROUTES.production` (`/production`). **Bottom/side nav (BUILT, `navItems.jsx` producer branch) is 4 tabs:** Team (`/production`) · Events (`/production/events`) · Requests (`/production/requests`) · Account (`/settings`). **Flagged correction:** the prior spec text claimed "Default = Events" — the code's actual default landing route (`/production`, what `RoleHome` sends a producer-workspace user to) renders the **Team** section (`sectionFromPath()` falls through to `'team'` for any path not ending `/events` or `/requests`). Team is the true default, not Events. **Out:** each section is a tab-switch within the same component (`<SectionTabs>`, in-page pill row) — never a page reload; `Members.jsx` (reached via "Manage team ›") carries its own back-to-`/` link.

**DESKTOP & MOBILE layout / no-scroll target.** `PageShell max-w-4xl`. Page head (H1 "Production" — actually `T.production.title`, generically "Production workspace" — + org-name subtitle) → a pill-style `SectionTabs` row (Team/Events/Requests, `rounded-full` segmented control, current tab filled lime) → the active section's content. **No-scroll target:** like Representation, this is a variable-length list page, not a bounded one-viewport canvas — the correct contained-scroll unit is the section's own card list (Events: one card per event with its slots nested inside; Requests: one card per request), which grows with real data rather than being clipped.

**COMPONENTS (BUILT, per section).**
- **Team** (`TeamSection`) — a read-only preview of org members: name + role chip per row (no invite/remove actions here — those live only at `/org/members`); empty state = "no team members yet" + a CTA into `/org/members`; populated state ends with "Manage team ›" linking out. This section duplicates a slice of Members.jsx as a preview, nothing more.
- **Events** (`EventsSection`) — one card per event (`groupGigsIntoEvents(listOrgGigs())`): header = event title + date/venue/city; below, each lineup slot = artist name + `role_at_event` label (headliner/support/lineup-member, or "role not set") + an `audience_band` `BandPill` (band-only, never an exact number, and only shown when the band isn't `'unknown'`) + a status chip (`gigStatus`: **lead · hold · confirmed · settled · canceled** — five states, not the three named in the prior spec draft). Empty state = "no events yet" + explanatory body.
- **Requests** (`RequestsSection`) — availability requests THIS workspace sent, via `listProductionRequests()` (a 032-gated RPC): `undefined`=loading, **`null`=an honest "not wired yet" gap card** (rendered when migration 032 isn't applied — never a guessed/invented row), `[]`=genuine empty, else a card per request (artist name · event type/location/date · quoted message · a colored status word: new=amber / replied=accent / closed=faint).

**BUILT vs the previous spec's interactivity claims — the honesty section (flagged, not invented, verified by reading every line of `ProductionDashboard.jsx`).** The prior spec described a "lineup-slot board" with **one CTA per slot** ("Confirm for this slot"), a "＋ New event / open slot" primary action, and a Requests screen where a production company "confirms a slot the moment an artist says yes." **None of these exist in the built component.** Concretely: (1) `EventsSection` renders slot chips with **zero click handlers** — no button, no link, nothing tappable on any slot; (2) there is **no create-event / open-slot UI anywhere** — not a button, not a sheet, not even a disabled placeholder; (3) `RequestsSection` renders request cards with **zero action buttons** — no reply, no confirm, no forward; it is a pure read list. The only interactive elements on the entire Production workspace are: the 3 section tabs, the "Manage team ›" outbound link, and the "no team yet" empty-state CTA to `/org/members`. This matches — and should be read as the authoritative restatement of — §7.7.a's own flag ("Production creation flows — event/slot creation UI is TARGET, states exist only as names") and this team's brief ("keep §8.11's C6-gated creation flows marked owner-gated"): **every slot-confirm, event-create, and request-reply interaction on this entity is TARGET, owner-gated, and not to be built from this pass.**

**ENUMERATED INTERACTIVITY — every interactive element that DOES exist today:**
| Element | Trigger | In-place behavior |
|---|---|---|
| Team / Events / Requests tabs | tap | route-switches (`Link` to the three real routes), pill highlight moves, no reload |
| "Manage team ›" (Team section, populated) | tap | navigates to `/org/members` |
| "no team yet" CTA (Team section, empty) | tap | navigates to `/org/members` |
| Event/slot chips (Events section) | — | **none — display only, no trigger** |
| Request cards (Requests section) | — | **none — display only, no trigger** |

**ENUMERATED INTERACTIVITY — TARGET (owner-gated, C6, named so a future build doesn't invent the shape):**
| Element (target) | Intended trigger | Intended in-place behavior |
|---|---|---|
| "＋ New event / open slot" | tap | sheet, never a new page (per §7.7 screen-count discipline) — Draft event card created |
| Slot chip, state=open | tap | "Confirm for this slot" → opens a Passport/act picker, books via the Buyer path (§8.8) |
| Suggested-act card | tap | fit *reason* text (never a rank) + opens that act's Passport |
| Request card reply | tap | confirms the slot the moment an artist says yes — currently has no UI hook at all |

**STATES.** Team: loading · error (retry) · empty · populated. Events: loading · error · empty ("no events yet") · populated (per-slot status chip only, no interaction). Requests: loading (`undefined`) · **gap state** (`null`, migration 032 not applied — an honest card, not a guess) · empty (`[]`) · populated (status-only, no interaction).

**SIGNALS FIRED — none.** `grep` of `ProductionDashboard.jsx` for `logEvent(` returns zero matches, consistent with this entity firing nothing in §7.7.a's own signals column ("Conversion (`availability_request_created` as buyer)" is the entity's INTENDED signal once the Buyer-path booking flow is built — it does not fire from anywhere in this component today, since that flow does not exist yet).

**LEXICON (EN + HE, `production.*` namespace — ✓ shipped).** Representative keys: `title` / `subtitle` / `subtitleFor(name)` / `teamTitle` / `teamEmpty` / `manageTeam` / `eventsTitle` / `eventsEmptyTitle` / `eventsEmptyBody` / `untitledEvent` / `roleAtEvent.{headliner,support,lineup-member}` / `roleUnset` / `gigStatus.{lead,hold,confirmed,settled,canceled}` / `audienceBand.*` / `requestsGapTitle` / `requestsGapBody` / `requestsEmptyTitle` / `requestsEmptyBody` / `reqStatusNew` / `reqStatusReplied` / `reqStatusClosed`. HE confirmed present verbatim (e.g. `title` → "מרחב הפקה", `teamEmpty` → "אין עדיין חברי צוות", `gigStatus.confirmed` → "מאושר"). As with §8.10, no `production.*` row set exists yet in §15.4 — flagged, out of this team's territory.

**FIREWALL / entity law.** Production never owns an artist's evidence; it reads Passports (a firewall statement that is currently vacuous in code, since no path in this component opens a Passport at all — the statement becomes true only once the Buyer-path booking target is built). No score/rank of acts; suggested acts would carry a fit *reason*, never a rank (target).

**DEFINITION OF DONE (today, BUILT).** Three tabs render real org-scoped data (members / grouped gigs+slots / sent requests); each section has honest loading/error/empty states, including a genuine "not wired yet" gap state for Requests when 032 is absent; zero invented rows, zero invented interactions. **DEFINITION OF DONE (TARGET, owner-gated — not authorized by this pass):** event/slot creation sheet; per-slot confirm → Buyer-path booking; request-reply action; suggested-act fit cards.

**PART-8 SHAPE.**
- **SUCCESS FORMULA.** Today, this screen succeeds simply by never lying — a production company sees exactly its own team, its own booked slots, and its own sent requests, with an honest "not wired yet" where the data path doesn't exist, never a guessed number. Once the target creation/booking flows land, success becomes: a production coordinator fills an open slot in under a minute without ever leaving the board.
- **COMPLETENESS CHECKLIST.** Nav-in/out incl. the corrected default-tab ✓ · every BUILT interactive element enumerated (five, total) ✓ · every TARGET interactive element named and marked owner-gated ✓ · states per section incl. the 032-gap state ✓ · signals fired (zero, matches §7.7.a) ✓ · EN+HE lexicon ✓ · firewall (currently vacuous, honestly stated) ✓ · no-scroll target (variable-length list, correctly not bounded to one viewport) ✓.
- **PARAMETER AUDIT (vs HEAD, this pass).** Taxonomy/terminology: **present** (`gigStatus` 5-state vocabulary, `role_at_event` vocabulary — both already canon in code, now surfaced in spec). Entities: **present** (production workspace = `organization.workspace_type='producer'`, §3.4, unchanged). Interactivity enumeration: **was badly THIN/wrong** (claimed a fully interactive slot board with CTAs and creation flows that do not exist) → **corrected in place**, `ratify: R00`, marked owner-gated per this team's brief — no code authorized. Nav map: **was THIN** (wrong default tab) → **corrected**. Signals: **present, matches §7.7.a** (no change needed). Design-system tokens: unchanged.

---

### 8.12 Admin / Operator cockpit (`/admin`)

_Deepened to the full interactive standard, 21 Jul — TEAM IX-ENTITIES. Grounded in the BUILT code: `AdminDashboard.jsx` (692 lines) + `gateCounts.js` (211 lines) + `navItems.jsx` (operator branch). The BUILT-vs-PENDING table below (last updated T-55, 18 Jul) is corrected in place — the AI-cost ledger has since shipped (T-92, 20 Jul) and the table below was stale._

**THE ONE JOB.** One internal person opens one long page and can answer, without guessing: has the Gate been reached, is the pilot funnel moving, is AI spend under control, and does anything here need a hands-on decision (activate a payment, approve an upgrade, honor a data-erasure request) — every number sourced, timeframed, and demo-excluded. **1-screen happy path (§6 law 13 — this IS the model target-canvas exemplar):** land on `/admin` → the sticky anchor bar offers 9 jump targets → scroll or jump to the section that needs a decision → act (one tap) → the row updates in place, no navigation away.

**PURPOSE.** An internal operational **cockpit** (not a pale dashboard): the Gate as hero metric, retention, a visual funnel, an honest AI-cost ledger, publish-freshness, and five operator queues — with every metric's **source event · timeframe · demo-excluded** made explicit. Internal LOCK team only; never reachable via public signup (`SIGNUP_ROLES` excludes `OPERATOR`, `constants.js`).

**NAV IN / OUT (per §7.7.a + `navItems.jsx`, verified).** **In:** `RoleHome` sends `role===OPERATOR` straight to `/admin` — this IS the operator's home, not a secondary screen. **Bottom/side nav (BUILT, `navItems.jsx` operator branch) is 2 tabs only:** Admin (`/admin`) · Account (`/settings`) — the simplest nav set in the product, matching an internal-only, single-canvas entity. **Out:** nothing to exit to; Account is the only other destination. **Within-page nav (BUILT) is a sticky anchor bar** (`sticky top-0 z-20`, `-mx-4` bleed, backdrop-blur) with 9 jump links — `gate` · `funnel` · `payments` · `upgrades` · `artists` · `requests` · `claims` · `consents` · `audit` — each an in-page `#hash` scroll (`scroll-mt-16` on the target `<section>`), never a route change. This is the one entity in §8 that is *already* built exactly to the §7.7 "one canvas, everything folds into anchors/tabs" target — worth citing as the model other entities should converge toward.

**DESKTOP & MOBILE — one continuously-scrolling page, no bounded viewport.** `PageShell max-w-2xl`, dark theme (this is the one non-Radar/Passport screen that stays dark — an internal ops console, not a buyer/artist-facing surface). **No-scroll target:** like Representation and Production, this is correctly NOT a one-viewport canvas — it is a genuinely long single page (9 sections), and the **named contained-scroll region is the sticky anchor bar itself** (`overflow-x-auto` on narrow width so 9 pills never wrap or get cut), giving instant re-orientation without a second scrollable panel anywhere else on the page.

**COMPONENTS (BUILT, top → bottom).** Eyebrow + H1 "Admin console" (`T.admin.title`) + subtitle · sticky anchor bar (9 links) · 5 auto-fit stat cards (artists/published/requests/new-requests/claims — plain inventory counts) · **§gate**: 5 Gate tiles (views/reactions/requests/intent/verified-pay, each with its exact source-event tag) + 2 Retention tiles (returning accounts / repeat Passport opens) · **§funnel**: 8-stage pilot-funnel fill-bars + 2 publish-freshness tiles + the AI-cost ledger (runs/30d tile + status breakdown + a deliberately non-numeric manual-spend card) · a static SEC-01 compliance checklist · **§payments**: pending-entitlement rows with "Mark active" · **§upgrades**: pending org-upgrade rows with "Approve" · **§artists**: paginated (20/page) artist rows with publish-toggle + view-Passport + export + delete · **§requests** / **§claims** / **§consents** / **§audit**: paginated read-only rows · a delete-confirm `BottomSheet` (reason required).

**ENUMERATED INTERACTIVITY — every interactive element, in place, no new page:**
| # | Element | Trigger | States | In-place behavior | Result |
|---|---|---|---|---|---|
| 1 | Anchor-bar link ×9 | tap | — | smooth in-page scroll to `#id` | no data change |
| 2 | Gate/Retention/Funnel/AI-cost "Retry" (on error) | tap | loading → success/error | re-runs that section's OWN fetch (`loadGate`/`loadRet`/`loadFunnel`/`loadAiCost`) | that section alone updates — a metrics hiccup never blanks the console (each has an isolated load/error/retry contract, explicit in code comments) |
| 3 | Payments row **"Mark active"** | tap | idle → in-flight → removed from list | `adminActivateEntitlement()` → `logEvent(EVENTS.ENTITLEMENT_ACTIVATED, {...})` → `loadGate()` re-runs so the verified-pay tile reflects it immediately → fire-and-forget artist notification | **this is the Gate's second half firing** — the ONLY client-side analytics call anywhere in this component (see Signals) |
| 4 | Upgrades row **"Approve"** | tap | idle → in-flight → removed from list | `approveUpgrade(orgId)` — flips plan+seats on the SAME org (Solo→Agency), no migration | row disappears from the queue |
| 5 | Artist row **publish toggle chip** | tap | on ⇄ off, disabled while `toggling` | `adminSetPublished()` | row's chip flips state in place |
| 6 | Artist row **"View Passport"** | tap | — | opens `/passport/{id}` (new context, the one deliberate navigate-away on this screen) | — |
| 7 | Artist row **"Export data"** | tap | — | `adminExportArtist()` → client builds a `Blob` → triggers a browser download (`lock-{stage_name}.json`) — OP12 right-to-access/portability | a file downloads; no page change |
| 8 | Artist row **"Delete data"** | tap | opens `BottomSheet` | reason field required (`deleteReason`), Confirm disabled until non-empty | `adminDeleteArtist(id, reason)` — writes an audit row BEFORE deleting (OP12 erasure); row removed from the list on success |
| 9 | "Show more" (5 sections: artists/requests/claims/consents/audit) | tap | — | `usePaged()` reveals 20 more rows client-side, no re-fetch | — |

**STATES.** Each of Gate/Retention/Funnel/AI-cost/publish-freshness carries its **own** loading/error/retry — a genuine architectural choice (code comment: "a metrics hiccup never blanks the console"), so one failed query never takes down the other seven sections. Artists/requests/claims/consents/audit: loading (whole-page) · error (whole-page retry) · empty (per-section `EmptyState`) · populated + paginated. Delete-confirm sheet: closed · open-reason-empty (Confirm disabled) · open-reason-filled · busy · error.

**SIGNALS FIRED — exactly one, client-side; the entity map's blanket claim needs a one-line correction.** `grep` of `AdminDashboard.jsx` for `logEvent(` returns **one match**: `EVENTS.ENTITLEMENT_ACTIVATED` inside `activate()` — the operator-side half of the Gate (§1.6, §14.4). §7.7.a's entity-map row for OPERATOR says "reads ALL families; fires none" — **that is not quite accurate**; Operator fires exactly this one canon event, and it is arguably the single most important write on this screen (it is one of the two conditions of the Gate itself). Every other number on this page is a **read** (via `gateCounts.js`'s five fetch functions, each `is_demo=false`-filtered except the AI-cost ledger — see next). **Flagged honesty note (verified in code comments, not invented):** `fetchAiRuns30d()` counts `processing_job` rows, which carry **no `is_demo` column and no client-reachable seed marker** — the AI-cost tile is the one metric on this page that is **not** demo/seed-excluded, and the code's own comment says so; the UI must keep stating this plainly (it already does, via `aiCostNote`) rather than implying the same guarantee as the Gate/Retention/Funnel tiles.

**LEXICON (EN + HE, `admin.*` namespace — ✓ shipped, extensive).** Representative keys: `title` / `subtitle` / `gateTitle` / `gateViews` / `gateReactions` / `gateRequests` / `gateRefs` / `gateActivated` / `gateTagContext` / `gateTagReaction` / `gateTagIntent` / `gateTagPaid` / `gateNote` / `gateError` / `gateRetry` / `retTitle` / `retReturning` / `retReturningTag` / `retRepeat` / `retRepeatTag` / `retNote` / `funnelTitle` / `funnelSignup…funnelRequested` (8 stage labels) / `funnelNote` / `freshTitle` / `freshPublished` / `freshUnpublished` / `freshNote` / `aiCostTitle` / `aiCostRuns` / `aiCostRunsTag` / `aiCostStatusQueued/Running/Completed/Failed` / `aiCostSpendLine` / `aiCostSpendTag` / `aiCostNote` / `sec01Title` / `sec01Items[]` / `paymentsTitle` / `markActive` / `noAmountNote` / `noEmailOnFile` / `upgradesTitle` / `approveUpgrade` / `artistsTitle` / `publishToggleOn/Off` / `viewPassport` / `exportData` / `deleteData` / `deleteConfirmTitle` / `deleteAuditNote` / `deleteReasonLabel` / `deleteReasonPlaceholder` / `deleteConfirmCta` / `requestsTitle` / `claimsTitle` / `consentsTitle` / `consentMarketing` / `auditTitle` / `showMore` / `actionError` / `anchors.{gate,funnel,payments,upgrades,artists,requests,claims,consents,audit}`. HE confirmed present verbatim (e.g. `title` → "קונסולת מנהל מערכת", `sec01Items[0]` → "RLS מופעל על כל הטבלאות — בידוד דייר (ארגון)"). As with §8.10/§8.11, no `admin.*` row set exists yet in §15.4 — flagged, out of this team's territory.

**FIREWALL.** Funnel + Gate + Retention = the user base's OWN product milestones (counts of events), never a grade of any artist — every tile is demo-excluded (`is_demo=false`) except the AI-cost ledger, which says so honestly in-copy. Intent (`payment_reference_created`) is willingness-to-pay, **never** counted as revenue — `entitlement_activated` is the only event that means "paid." The capped-50-row claims/audit lists are pagination, never a leaderboard. OP12 export/delete both operate on one artist's full record at a time (Amendment-13 aligned, §15.1) — delete requires a typed reason and writes an audit row first, never a silent erasure.

**DEFINITION OF DONE.** Gate hero tiles + Retention tiles with exact source events, each independently loading/error/retry; visual funnel (counts only, own-max fill bar) with per-stage source event shown in mono; AI-cost ledger with an honest non-demo-excluded disclosure (no invented spend number); publish-freshness (published/unpublished counts, per-item staleness explicitly flagged as needing a future read model); 5 operator queues with pagination; publish-toggle, entitlement-activate, upgrade-approve, export, and reasoned-delete all mutate in place; sticky anchor nav; internal-only, 2-tab nav shell.

**BUILT-vs-PENDING per tile (audit T-55 · 18 Jul 2026, corrected in place 21 Jul — the AI-cost row was stale):**
...
| **Retention tiles ×2** (returning accounts · repeat Passport opens) | ✅ **BUILT (T-55)** — `fetchRetention()`, demo-excluded, own load/error/retry | `login` (incl. `session-restore`) · `passport_view` + `return_visit` — FIRING |
| Stat counts (artists · published · requests · claims) | ✅ BUILT | table reads (inventories, not demand metrics) |
| Pilot funnel bar (signup → … → availability_request) | ✅ **BUILT (Wave-B/T-81)** — 8 stages driven by `FUNNEL_EVENTS` in `gateCounts.js` (render cannot drift from canon), `is_demo=false`, proportional fill vs the funnel's own max, source event in mono per stage | canon events — FIRING |
...
| **AI-cost ledger** | ✅ **BUILT (T-92, owner ruling "honest hybrid," 20 Jul — corrects the stale PENDING mark below)** — `processing_job` run-count/30d + status breakdown are REAL; spend is a manually-tracked, deliberately non-numeric line (no server-side cost read path exists yet); **NOT demo/seed-excluded** (flagged above), stated honestly in-copy | `processing_job` table read (022) |
| Risk tile | ⏳ **still PENDING** — correctly FLAGGED OUT, not faked: needs an aggregate query (a capped 50-claim list would be dishonestly partial) | — |

**PART-8 SHAPE.**
- **SUCCESS FORMULA.** This cockpit succeeds when the operator can answer "have we hit the Gate, and does anything need my hand right now" in one scroll, trusting every number because each one states its own source, window, and demo-exclusion — it fails the moment any tile looks precise but isn't (the AI-cost tile is the test case: it must never LOOK like a real-time budget dial when it is a manual line).
- **COMPLETENESS CHECKLIST.** Nav-in/out (2-tab shell, 9-anchor in-page nav) ✓ · every interactive element enumerated (9, incl. the one signal-firing action) ✓ · per-section independent states ✓ · signals fired incl. the one-line correction to §7.7.a's "fires none" ✓ · the AI-cost demo-exclusion honesty gap ✓ · EN+HE lexicon ✓ · firewall (counts not grades, intent≠revenue) ✓ · no-scroll target (long page + sticky anchor bar as the contained-scroll unit) ✓ · the BUILT-vs-PENDING table corrected for T-92 ✓.
- **PARAMETER AUDIT (vs HEAD, this pass).** Taxonomy/terminology: **present** (Gate/Retention/Funnel/AI-cost vocabulary already canon, §14). Entities: **present** (Operator, §3.4/§4.2, unchanged). Interactivity enumeration: **was THIN** (no element-by-element table existed; the one signal-firing action wasn't called out) → **backfilled in place**, `ratify: R00`. Nav map: **present, unchanged** (2-tab shell already matched code). Signals: **was slightly wrong** ("fires none") → **corrected in place** this pass. BUILT-vs-PENDING table: **was stale** (AI-cost still marked fully PENDING after T-92 shipped it) → **corrected in place** this pass. Design-system tokens: unchanged.

---

### 8.13 Shared / supporting screens

_Deepened to the full interactive standard, 21 Jul — TEAM IX-ENTITIES. Four screens bundled under this heading (Buyer/discover is the primary owner-flagged gap; Evidence capture, Claim review, and Auth/redirectors are the surviving "supporting screens"). Grounded in the BUILT code: `BookerHome.jsx` (84 lines), `EvidenceCapture.jsx` (388 lines), `ClaimReview.jsx` (519 lines), plus `navItems.jsx`/`navigation.js`/`App.jsx` for nav facts. Auth screens themselves (login/signup/reset/invite) already have their full §17.B.1–4 treatment — this section states only their nav-in/out relationship to the rest of §8, not a duplicate spec._

#### 8.13.1 Buyer signed-in home (`/discover`) — the owner-flagged THIN screen, now deepened

**THE ONE JOB.** A signed-in professional buyer opens exactly one Passport they already have a link or ID for — nothing else is possible or needed here, since a buyer otherwise needs no account at all. **1-screen happy path:** land on `/discover` → paste a Passport URL or ID → tap "Open passport" → land on `/passport/{id}` (public, no-shell context, §8.7).

**PURPOSE.** Buyers are **trust-receivers**, not browsers: they open a Passport from a link they were sent. Open marketplace/discovery is deliberately deferred (post-validation, §1.6) — this screen exists only so a signed-in buyer (one who self-selected "get context and clarity on an artist before booking" at signup) has a home to land on and a way to re-open a Passport without needing the original message again.

**NAV IN / OUT (per §7.7.a + `navigation.js` + `navItems.jsx`, verified).** **In:** two paths — (a) `selectRoute(ROLES.BOOKER)` sends a brand-new self-selecting signup straight here (`navigation.js` line 57); (b) `RoleHome`/`homePathFor` sends any returning signed-in booker to `ROUTES.discover` (`/discover`) as their home. **Bottom/side nav (BUILT, `navItems.jsx` booker branch) is 2 tabs:** Discover (`/discover`) · Account (`/settings`) — the second-simplest nav shell after Operator's. **Out:** the ONLY forward path is opening a Passport, which navigates to `/passport/{id}` — a genuinely different context (no login, no AppShell, no hub, per §7.6). Returning to `/discover` afterward requires the browser back button or the bottom-nav Discover tab; there is no "recently opened Passports" list to return to a prior one (a real, if minor, gap — the sample-passport link is the only one-tap re-entry, and only for the demo fixture, never for a real Passport the buyer previously opened).

**DESKTOP & MOBILE — single viewport, no scroll target.** `PageShell max-w-md`, identical narrow-column shape at 390px and 1360px (deliberately not full-width even on desktop — a premium, focused entry, not a dashboard). Two stacked cards, no page scroll at either width: (1) the hero — eyebrow "Booking manager" + H1 + subtitle + one labeled input + one primary CTA + an escape-hatch line; (2) a secondary "What is a LOCK Passport?" card with a bulleted explainer + the legal disclaimer line, both light-theme, no dark island (this entity never touches Radar/Passport atmosphere).

**COMPONENTS (BUILT, `BookerHome.jsx`).** Hero card with a decorative radial-gradient wash (`aria-hidden`, purely atmospheric) · `Field` (labeled input, `dir=ltr`, Enter-to-submit) · primary CTA ("Open passport") · an inline error (`T.booker.pasteError`) on empty/unrecognized input · escape-hatch line ("No link yet? Try a sample passport") · secondary explainer card (`whatIsTitle` / `whatIsBody` / a checklist of `whatIsPoints[]`, each with a lime ✓ glyph) · a footer disclaimer line (`T.authScene.disclaimer`, shared with the auth screens).

**ENUMERATED INTERACTIVITY — every interactive element:**
| # | Element | Trigger | States | In-place behavior | Result |
|---|---|---|---|---|---|
| 1 | Passport-link input | type / paste / Enter | empty → filled; error clears on any keystroke after a failed attempt | regex-parses `passport/([0-9a-f-]{6,})` out of a pasted full URL, or takes the raw value as an ID | value staged for open |
| 2 | **"Open passport"** (primary) | tap, or Enter in the field | idle → navigate, or idle → inline error | if a usable ID is found → `navigate('/passport/{id}')`; else sets `T.booker.pasteError` inline, field stays focused | success = full navigation to the public Passport (leaves this screen entirely) |
| 3 | **"Try a sample passport"** escape hatch | tap | — | `Link` to `/passport/demo-artist` | opens the fixed demo Passport — the one always-available "recently opened" equivalent |

**STATES.** Default (empty field) · filled · error (pasted text didn't resolve to an ID) · navigating (browser-level, no in-page spinner needed — the action IS leaving the page).

**SIGNALS FIRED — none.** `grep` of `BookerHome.jsx` for `logEvent(` returns zero matches. Opening a Passport from here does **not** distinguish itself from any other entry path in the analytics stream — the eventual `passport_view` fires only once the destination page mounts, carrying no marker of "came from the signed-in booker home" (contrast with the `?s=1` share-link marker, §7.6, which DOES tag its origin). **Flagged gap:** if the product ever wants to know how much Passport traffic comes from returning signed-in buyers vs fresh share links, this screen needs to attach a first-party origin marker before navigating — not built today.

**LEXICON (EN + HE, `booker.*` namespace — ✓ shipped, already documented at §15.4.6).** Full table lives there; representative keys used on this exact screen: `eyebrow` / `title` / `subtitle` / `inputLabel` / `openCta` / `pasteError` / `noLinkYet` / `sampleCta` / `whatIsTitle` / `whatIsBody` / `whatIsPoints[]`. §15.4.6 already flags the one open gap relevant here: **◐ PENDING** — no warmer, non-industry copy register exists yet for a private/corporate signed-in buyer (this screen's copy is written for a professional booking manager only); a private-client variant of `/discover` is a **named, not-yet-built** adjacent gap, distinct from and in addition to the Passport-side private/corporate register gap already tracked at §8.7 (D9).

**FIREWALL.** No evidence surface lives on this screen — it is a pure resolver + explainer. The explainer copy is already firewall-clean ("Method-labeled claims", "Draw as bands and binaries — no rankings, no predictions").

**DEFINITION OF DONE.** One input + one CTA resolves a pasted URL or raw ID to a Passport; inline error on an unresolved paste; sample escape hatch always available; explainer card states the firewall in buyer-readable language; 2-tab nav shell; one viewport, no scroll, at both widths.

**PART-8 SHAPE.** **Success formula:** a booking manager who was sent a link days ago can find their way back to it (or a placeholder) in one screen, without needing the original message. **Completeness checklist:** nav-in/out (2 entry paths, 2-tab shell) ✓ · 3 interactive elements enumerated ✓ · states ✓ · signals (none — origin-tracking gap flagged) ✓ · EN+HE lexicon (cross-referenced to §15.4.6) ✓ · firewall ✓ · one-viewport no-scroll ✓. **Parameter audit (vs HEAD):** taxonomy/entities present (Booker, §3.5, unchanged); interactivity enumeration **was THIN** (one line in the prior spec) → **backfilled in place**, `ratify: R00`; nav map **was THIN** (the §7.7.a "FLAGGED THIN… one line in §8.13" note is resolved by this pass); signals **newly stated** (was silent); design-system tokens unchanged.

---

#### 8.13.2 Evidence capture (`/evidence/:artistId`)

**THE ONE JOB.** Turn ONE thing the artist wants to prove (a claim-intent) into one piece of stored, method-labeled evidence, via the shortest path for that intent — upload, connect, or declare. **1-screen happy path:** arrive from the Radar (a planet's "add" affordance or next-action dock) → pick a path card → pick the specific intent → fill the matching micro-form → submit → toast confirms → back to the Radar.

**PURPOSE.** The claim-first canon (A7): the artist starts from WHAT they want to prove, and the system asks for the matching evidence — never a generic "upload a file" dead end. Three capture paths, each mapped to specific claim intents: **Upload** a document (`drew-crowd`, `sold-via-link`, `produced-event`) · **Connect** a source (`rebooked`, `consistent-frequency`, `producer-confirm`) · **Declare** a band (`community`).

**NAV IN / OUT (verified, `App.jsx` line 200 + code).** **In:** `RequireRole role={ARTIST}` only; reached exclusively from within the artist workspace (the Radar's planet panel / next-action dock, per §8.2/§8.3 — not a nav-bar destination, matching the §7.7.a Artist row: "Evidence §8.13 (planet panel → back to Radar)"). **Out:** an explicit "← back" link to `/artist/home` (confirmed twice in code, lines 167 and 187) — never a dead end, always returns to the Radar in place.

**DESKTOP & MOBILE / no-scroll target.** `PageShell`, light theme. Two-phase layout: (1) the three path cards (a bounded selection grid, fits one viewport at both widths); (2) once an intent is picked, the per-intent micro-form replaces the cards in place (not a new page) — this is the named contained region that changes, the page shell around it stays fixed.

**COMPONENTS (BUILT).** Consent gate (`hasConsent('evidence-storage')`, collected inline the first time this screen is used, not upfront at signup — "Accept" button) · 3 path cards (`upload ⇪` / `connect ↗` / `declare ✎`) · a live platform-recognition hint (`detectPlatform()` + `PlatformLogo`) that names the platform the moment a link is pasted, mirroring the Radar's own smart-link behavior (§9.8) · per-intent micro-forms: file input (upload paths), band select + optional public URL (declare/some connect paths), free text, numeric field · a toast (`flash()`, ~2.2s) on success.

**ENUMERATED INTERACTIVITY:**
| Element | Trigger | In-place behavior |
|---|---|---|
| Evidence-storage consent "Accept" | tap | `recordConsentScope(user.id, 'evidence-storage')`; gate clears, forms become usable |
| Path card (Upload/Connect/Declare) | tap | reveals that path's intent list, in place |
| Intent chip | tap | sets `intent`, reveals the exact micro-form for that intent |
| Link/URL field | paste | `LinkPlatformHint` shows the recognized platform + logo live, before submit |
| File input | choose file | client-side validated (`evidenceFileError` — size/type); bad file → inline error, field resets; good file → `uploadFile()` → `submitEvidence()` |
| Band select (+ optional URL) | select / type | "Add" submits `submitEvidence({evidence_type:'band', source_type: url?'public-profile':'self-band', ...})` |
| Submit (any path) | tap | `addEvidence()` writes the item tagged with `claim_intent` + `source_owner_consent:true`; forms reset; list reloads; toast fires |

**STATES.** Consent: ungranted (gate shown) → granted (forms usable). Path-select → intent-select → form (empty/typing/uploading/error/success, per the §10.4 per-field DoD). File: none → selected → uploading (busy) → error (size/type, human message) → added.

**SIGNALS FIRED (verified, `EVENTS` in `analytics.js`, both CANON, FIRING).** `EVENTS.EVIDENCE_UPLOADED` (`evidence_added`) on every successful submit, tagged `{artist_id, evidence_type}` — code comment marks this explicitly "pilot signal (A10)". `EVENTS.EVIDENCE_PROCESSED` (`gig_evidence_refresh_completed`) fires once the AI extraction pass completes on that item. Both are Build-family funnel events already reflected in the Admin funnel (§8.12) and the Radar's own milestone ladder (§8.2 M2/M3).

**LEXICON (EN + HE, `evidence.*` namespace — shipped; not yet in §15.4).** Representative keys: `title` / `platformRecognized(name)` / `addedOk` / `errFileSize` / `errFileType` / `paths.{upload,connect,declare}.{title,desc}`. HE presence not independently re-verified line-by-line this pass (time-boxed); flagged for the next pass to confirm against `he.js` if this screen is picked up for a build task.

**FIREWALL.** Nothing publishes until the artist confirms it in Claim review (below) — evidence capture only stores + method-labels; it never marks anything `passport_ok`. A pulled/connected source is checked against its own origin, never re-hosted as an independent claim.

**DEFINITION OF DONE.** Claim-first 3-path selector; consent collected inline, not upfront; live platform recognition on paste; per-intent micro-form with the full per-field DoD; both funnel signals firing; explicit back-to-Radar link; no dead ends.

**PART-8 SHAPE.** **Success formula:** the artist never has to guess what file/link/text to add — the path they picked already told the system what it's for, and the form matches. **Completeness checklist:** nav-in/out ✓ · interactivity enumerated ✓ · states ✓ · signals (2, both firing) ✓ · lexicon (HE flagged unverified this pass) ✓ · firewall ✓ · no-scroll (2-phase in-place swap) ✓. **Parameter audit (vs HEAD):** interactivity enumeration **was a one-line bullet** → **backfilled**, `ratify: R00`; everything else (taxonomy, nav, signals) was already accurate in the one-line form, now made explicit.

---

#### 8.13.3 Claim review (`/artist/claims`)

**THE ONE JOB.** Decide, per extracted claim, whether it is right, needs a fix, should be hidden, is disputed, or is ready to carry to the public Passport — and optionally hand it to a Source-Confirmer for the strongest possible label. **1-screen happy path:** land on a claim → approve (or correct/omit/flag) → the visibility toggle becomes available → optionally generate a `/confirm/:token` link for a producer → done, all in place on the same row.

**PURPOSE.** The A8 artist-approval gate: nothing reaches the public Passport that the artist has not explicitly approved, and this is the ONE screen where that approval, correction, dispute, and producer-escalation all happen, per claim, without leaving the row.

**NAV IN / OUT (verified — matches the existing App.jsx comment exactly).** **In:** NOT a nav-bar destination (removed from the artist tab bar; `App.jsx` comment: "reached from the radar next-action card and notifications"). The route survives only for those deep links. **Out:** explicit "back" link to `/artist/home` (line 177) plus, when claims exist, a "View public Passport" link to `/passport/{artist.id}` (a deliberate navigate-away to see the true buyer read).

**DESKTOP & MOBILE / no-scroll target.** `PageShell`, light theme. A genuinely variable-length list (unpublished-changes banner → draw-bands card → needs-review claims → passport-ok claims → mirror-only claims → track-record items) — correctly NOT a bounded one-viewport canvas; each claim row is the contained unit that expands its own correction field in place, never a separate page.

**COMPONENTS (BUILT).** Unpublished-changes banner (shown only when `artist.published && dirty`) with a "Republish" CTA (`publishPassport()`, clears the dirty flag) · a draw-bands card (always-public fields, edit-elsewhere hint pointing at the Radar) · `ClaimRow` per claim (claim wording bold · claim-type context line · method-label + concrete source reference + reviewed date · visibility toggle chip · Approve/Correct/Omit/Flag actions · the producer-confirmation-link generator) · `ItemRow` per track-record item (method label + date + visibility toggle only, no approve/correct/omit — items are already source-verified) · a named receipt toast (states WHAT was confirmed and WHERE it now appears, 3.4s).

**ENUMERATED INTERACTIVITY — every claim-row element:**
| # | Element | Trigger | In-place behavior | Result |
|---|---|---|---|---|
| 1 | **Approve** | tap | `updateClaim({artist_approved:true})` → `logEvent(CLAIM_CONFIRMED)` → a 420ms `litpop` bloom on the row → named receipt toast ("Added to your Passport" / "Saved privately") | claim moves from needs-review into passport-ok or mirror-only, per its verification status |
| 2 | **Correct value** (inline) | tap "Correct" → type → save | `updateClaim({value, verified_by:'artist'})`; if already passport-ok, marks the Passport dirty | wording updates in place; a stale published Passport now shows the republish banner |
| 3 | **Omit** | tap | `deleteClaim()` — "it just won't show", never counted against the artist | row disappears; if it was passport-ok, marks the Passport dirty |
| 4 | **Flag** (dispute) | tap | `updateClaim({status:'disputed', artist_approved:false, visibility:'mirror_only'})` | claim routes to an operator dispute queue (not independently confirmed as a visible Admin surface this pass — flagged below); never publishable while flagged |
| 5 | **Visibility toggle** (Passport-ok ⇄ Mirror-only) | tap | disabled unless `canPublish` (`artist_approved && verification_status ∈ {verified, supporting}`) — the UI never allows a self-reported or not-assessable claim to cross | `updateClaimVisibility()`; marks Passport dirty |
| 6 | **Generate producer-confirmation link** | tap (optional email field first) | `POST /api/request-confirmation` → `logEvent(PRODUCER_CONFIRMATION_SENT)` → link persisted to `localStorage` (survives navigation) → "Copy link" | produces the exact `/confirm/{token}` URL that is §8.9's entire entry point — this screen is where that link is BORN |
| 7 | **Item visibility toggle** (track-record) | tap | `updateItemVisibility()`; `logEvent(CLAIM_VISIBILITY_CHANGED)` (dev-only, see Signals) | Passport-ok ⇄ Mirror-only |

**STATES.** Loading · error (retry) · empty (no claims/items — CTA into Evidence capture) · populated, split into 3 buckets (needs-review / passport-ok / mirror-only) rendered as separate labeled groups, count in the group header. Per-row: default · correcting (inline field open) · toggling (busy) · bloom (post-approve, ~420ms) · confirmation-link idle/generating/ready/copied/error.

**SIGNALS FIRED — three, with one genuine measurement ambiguity flagged.** `CLAIM_CONFIRMED` (`claim_confirmed`, CANON, FIRING) on Approve. `PRODUCER_CONFIRMATION_SENT` (`producer_confirmation_sent`, CANON, FIRING) on link generation. `CLAIM_VISIBILITY_CHANGED` fires on both claim- and item-visibility toggles but is **dev-only** — it is not in the `analytics.js` CANON set, so it reaches the localStorage ring buffer only, never the DB (confirmed by reading the CANON list — this exact name is absent from it, and the code's own `EVENTS` block comments it under "dev-only (NOT persisted)"). **Flagged measurement ambiguity (genuine, not invented):** `claim_confirmed` is written from **two different real-world triggers that share one event name** — here, when the **artist** approves their own extracted claim (client-side, this screen); and, separately, in `server/index.js`, when a **producer** answers "yes" on a `/confirm/:token` ceremony (§8.9, server-side). The Admin funnel's `funnelClaim` stage (§8.12) counts both together and cannot distinguish "the artist self-approved" from "a third party externally confirmed" — two meaningfully different trust events collapsed into one counter. Worth a T-number (a distinct event name or a `via` property) if the funnel is ever used to reason about third-party-confirmation rate specifically.

**LEXICON (EN + HE, `claims.*` namespace — shipped; not yet in §15.4).** Representative keys: `title` / `subtitle` / `needsReview` / `needsReviewHint` / `passportOk` / `mirrorOnly` / `itemsTitle` / `alwaysVisible` / `drawTitle` / `drawNote` / `drawEditHint` / `confirmReceipt(dest, wording)` / `noClaimsYet` / `applyTitle` / `applyBody` / `applyCta`. Plus the shared `producer.*` keys reused here for the link-generation card (`requestTitle`, `requestHelp`, `requestCta`, `linkReady`, `copyLink`, `copied`, `pendingChip`, `serverOffline`). HE presence for `claims.*` not independently re-verified line-by-line this pass; flagged for confirmation at build time.

**FIREWALL.** Only `verified`/`supporting` + artist-approved claims can ever reach `passport_ok` — enforced client-side (`canPublish`) and (per §13.5) server-side too. A disputed claim is force-reset to `mirror_only` and un-approved. The receipt toast names the true destination — never a silent "saved" with no indication of where.

**DEFINITION OF DONE.** Approve/Correct/Omit/Flag per claim; visibility toggle gated to verified/supporting+approved only; the producer-confirmation link generator is the verified birthplace of §8.9's entry point; named receipt with true destination; dirty-Passport republish banner; back-to-Radar link; the two-source `claim_confirmed` ambiguity documented (not fixed, spec-first).

**PART-8 SHAPE.** **Success formula:** the artist can look at any claim and know, in one glance, exactly what a buyer would see if they approved it right now — never left guessing whether "approve" means "publish." **Completeness checklist:** nav-in/out ✓ · 7 claim-row interactions + item-row toggle enumerated ✓ · states ✓ · signals incl. the flagged `claim_confirmed` dual-source ambiguity ✓ · EN+HE lexicon (HE unverified flagged) ✓ · firewall (verified/supporting gate) ✓ · no-scroll target (variable list, correct) ✓. **Parameter audit (vs HEAD):** interactivity enumeration **was a one-line bullet** ("approve/edit/route… primary action = send to a מאשר-מקור") → **backfilled in place**, `ratify: R00`, including the previously-undocumented Omit/Flag actions and the dual-source signal ambiguity; nav map already accurate; taxonomy/entities unchanged.

---

#### 8.13.4 Auth / redirectors — nav relationship only (full spec at §17.B.1–4)

Login, signup (self-select role via `/select` → `UserTypeSelect`), reset/forgot password, and invite-accept (`/invite/:token`) already carry their complete PURPOSE/STATES/INTERACTIONS/MICROCOPY/FIREWALL/DoD treatment at **§17.B.1 (Signup) · §17.B.2 (Login) · §17.B.3 (Forgot/Reset) · §17.B.4 (Invite→Accept)** — not duplicated here. What belongs in §8.13 is their **nav relationship to the rest of §8**, verified against `App.jsx` and `navigation.js`:
- All four render **outside `<AppShell>`** (`App.jsx` lines 150–154, 162) — no hub, no per-entity nav, matching every other accountless/pre-auth surface in this product (Source-Confirmer §8.9, public Passport §8.7).
- **Deep-link honesty (verified, `RequireAuth`/`RequireRole`, `App.jsx` lines 52–58, 70):** any gated route hit while logged out stashes `state.from` and redirects to `/login`; `postLoginPath()` (`navigation.js` line 94) returns straight to that path after a successful login — the one navigation contract this whole product leans on to avoid a dead-end after auth.
- **New-signup routing (verified, `navigation.js` `selectRoute()`):** a fresh signup with no role yet lands at `/select`; picking ARTIST → `/onboarding` (§8.1) · AGENCY → `/agency` (§8.10) · BOOKER → `/discover` (§8.13.1, above). Producer and Operator are **never** self-selectable at signup (`SIGNUP_ROLES` excludes both) — a producer/operator account is always provisioned, never chosen.
- **Invite-accept** (`/invite/:token`) is the one auth-adjacent screen that may be reached either signed-in or signed-out — its own §17.B.4 spec covers both branches; nothing to add here beyond confirming it sits outside `<AppShell>` alongside the other three.

No new interactivity, states, or lexicon are introduced by this cross-reference — it exists solely so a reader following §8 end-to-end has the nav thread closed without re-deriving it from §17.B.
