...
- **Dead-link states (BUILT Wave-B/T-78, 20 Jul — ratify: R00).** Four warm terminal reads, each ending with a close-note exit line (no dead ends): **expired** ("short-lived on purpose — ask for a fresh one") · **used** ("already did its job") · **revoked** ("the artist cancelled it") · **invalid** (anything else). **Server contract (verified, `server/index.js`):** the API errors ONLY with 404=invalid and 410=expired; a used link returns 200 with its recorded receipt (idempotent single-use, G15-2) and a revoked link returns 200 and re-opens the ceremony — the deliberate re-confirmation path. A submitting state shows a spinner on the exact button pressed.
