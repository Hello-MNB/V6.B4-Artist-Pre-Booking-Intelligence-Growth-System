...
| Demo fixtures (`src/lib/demo.js`) | Complete | Complete | **100%** (50 of 50 `L(en, he)` bilingual-getter call sites have a non-empty HE argument) | No empty-string or missing HE arguments found in any `L(...)` call. File's own header comment claims language-purity-by-construction (EN mode renders only EN getters, HE mode only HE getters, brand names stay Latin in both) — spot-checked against the code and holds up (e.g. flagship fixture "Shai Perlman"/"שי פרלמן" — this is the literal, current code value; **not** the canonical demo-persona name — owner ruling 21 Jul 2026 sets that to "Maya Vale", see LOCK-PRODUCT-SPECIFICATION.md §8.4 — `src/lib/demo.js` has not yet been renamed, reported as a known gap there — · "Underground Techno"/"טכנו אנדרגראונד"). | LOW — this surface is a model for how the rest of the app should work; no action needed. |
