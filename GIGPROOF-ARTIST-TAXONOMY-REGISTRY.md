...
- **REGISTRY A — ARTIST TAXONOMY:** describes THIS ACT. One row per Act (re-keyed to `act_id` — multi-Act canon: one artist/Person may hold several Acts, each with its own taxonomy row, its own evidence, and its own Passport). Its columns are the TAGS (DJ, wedding, psytrance…). → decides WHICH fields are active.
- **REGISTRY B — FIELD APPLICABILITY:** describes EACH DATA FIELD. One row per field × module (module = legacy M-key; F1–F6 per-Act re-key pending). Its columns are the RULES. → decides HOW each field behaves.
- Link: Registry A's `primary_module` + tags → look up Registry B → get the Act's live field set.

---

# REGISTRY A · ARTIST TAXONOMY — columns (one row = one Act, keyed by `act_id`)
*The metafields that tag an Act (per-Act, non-transferable — one Person may hold several Acts). Multi-select where noted. This is what the artist/AI fills at onboarding.*

| # | Column (metafield) | Type | Values / notes | Firewall note |
|---|---|---|---|---|
| A01 | `artist_id` | uuid | stable key — re-key to `act_id` (registry is per-Act, not per-artist) | — |
| A02 | `stage_name` | text | — | passport-ok |
| A03 | `legal_entity` | text | for contracts/invoicing | internal |
| A04 | `primary_module` | single-select | M1–M8 (legacy — see supersession banner) | drives field set |
| A05 | `secondary_module` | single-select nullable | M1–M8 (legacy); cannot inherit primary's evidence | — |
...
| A17 | `active_overlays[]` | multi-select | O1–O10 (legacy — see supersession banner) | switches extra fields |
...
**8 MODULES (A04/A05 — legacy, see supersession banner):** M1 DJ/Producer-DJ · M2 Solo Vocal · M3 Instrumentalist/Session · M4 Band/Ensemble · M5 Large Ensemble · M6 Live-Electronic/Hybrid · M7 Service/Event Act · M8 Creator/Non-Performing.
**10 OVERLAYS (A17 — legacy, see supersession banner):** O1 Ticketed-Touring · O2 Festival · O3 Resident · O4 Content-First · O5 Original-Music · O6 Cover/Tribute · O7 Cultural/Traditional · O8 International/Export · O9 Brand/Sponsorship · O10 Educational/Family.

---

# REGISTRY B · FIELD APPLICABILITY — columns (one row = one field × module)
*The metafields that configure EACH data field. This is the big table GPT builds. Every atomic field in the 18 segments gets one row per module (legacy M-key — F1–F6 per-Act re-key pending).*
...
| B05 | `artist_module` | ref | M1–M8 (legacy — which module this row is for) | — |
...
| B16 | `next_action_rule` | text | the coach action when gap present | Artist view only (private — surfaced in the Artist Radar) |
| B17 | `visibility` | enum | passport-ok · working-only · internal | firewall |
...
1. Act row (A): `primary_module=M1` (legacy M-ID), `functional_subtype=[wedding-DJ]`, `commercial_model=[service-led]`, `genre=commercial`.
...
**Next Action** — Approve these two column sets. Then GPT builds Registry A as an Act-tagging form + Registry B as the field-config sheet, seeded with M1 (legacy ID) DJ/Producer-DJ (SHIGAON) — every 18-segment field gets its M1 row with all 24 B-columns filled.
